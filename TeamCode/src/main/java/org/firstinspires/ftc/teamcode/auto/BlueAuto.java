...
import static org.firstinspires.ftc.teamcode.auto.paths.IntakePoseToShootPose;
import static org.firstinspires.ftc.teamcode.auto.paths.StartPose2;
import static org.firstinspires.ftc.teamcode.auto.paths.IntakePose2;
import static org.firstinspires.ftc.teamcode.auto.paths.PerChance1;
import static org.firstinspires.ftc.teamcode.auto.paths.LastBall;
import static org.firstinspires.ftc.teamcode.auto.paths.Bald;
import static org.firstinspires.ftc.teamcode.auto.paths.Balls;
import static org.firstinspires.ftc.teamcode.auto.paths.StartPoseToShootPoseLast;
...
@Autonomous(name = "BlueAuto", group = "4848")
public final class BlueAuto extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        final Pose startPose = new Pose(25.121, 127.402, Math.toRadians(136));
        kaze.init(startPose);
            ...
            switch (State){
                ...
                case idle:
                    ...
                    if(!robot.drive.isBusy()) {
                        robot.drive.followPath(StartPoseToShootPose, true);
                ...
                case shooting1:
                    if(robot.stateTime.seconds() >= 4){
                        ...
                        robot.drive.followPath(ShootPoseToStartPointPose,true);
                ...
                case ShootPosetoIntake1:
                    if(!robot.drive.isBusy()){
                        ...
                        robot.drive.followPath(StartPoseToIntakePose,true);
                    }
                    break;
                ...
                case ShootPosetoIntake3:
                    if(robot.stateChanged){
                        robot.drive.followPath(StartPose2, true);
                        robot.spanker.setPosition(.65);
                    }
                    if(!robot.drive.isBusy()){
                ...
                case Intake3toGreenBall:
                    if(robot.stateChanged){
                        ...
                        robot.drive.followPath(IntakePose2, true);
                    }
                    if(!robot.drive.isBusy()){
                ...
                case GoBackFool:
                    if(robot.stateChanged){
                        robot.drive.followPath(PerChance1, true);
                    }
                    if(!robot.drive.isBusy()){
                ...
                case ShootingPose3:
                    if(robot.stateChanged){
                        robot.intake.setPower(0);
                        robot.spanker.setPosition(.65);
                        robot.drive.followPath(StartPoseToShootPoseLast, true);
                    }
                    if(!robot.drive.isBusy()){
                ...
                case parking:
                    if(robot.stateChanged){
                        robot.drive.followPath(LastBall, true);
                    }
                    if(!robot.drive.isBusy()){
                    ...
                    case Balls:
                        if(robot.stateChanged) {
                            robot.drive.followPath(Bald, true);
